...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--all", dest="download_all", action="store_true", help="Tai tat ca video tim duoc"
    )
    parser.add_argument("--url", help="Tai video tu bat ky URL nao (YouTube, TikTok, ...)")
    parser.add_argument(
        "--upload-video", help="Duong dan toi file video can upload len TikTok"
    )
    parser.add_argument(
        "--upload-desc", help="Mo ta/ caption khi upload len TikTok"
    )
    parser.add_argument(
        "--upload-cookies", help="File cookies (JSON) dung cho upload TikTok"
    )
    parser.add_argument(
        "--upload-sessionid", help="Gia tri cookie 'sessionid' dung cho upload TikTok"
    )
    ...
    parser.add_argument(
        "--request-timeout", type=int, help="Thoi gian timeout mang (giay)"
    )
    parser.add_argument(
        "--quick", action="store_true", help="Che do nhanh (giam mau sac trong shell)"
    )
    parser.add_argument(
        "--privacy", action="store_true", help="An thong tin IP tren banner"
    )
    parser.add_argument(
        "--metadata", choices=["json", "csv"], help="Xuat metadata kem theo video"
    )
    parser.add_argument(
        "--thumbnails", action="store_true", help="Tai thumbnail cho tung video"
    )
    parser.add_argument(
        "--playlist",
        action="store_true",
        help="Xuat file playlist (.m3u) chua link video",
    )
    parser.add_argument(
        "--rate-limit", type=int, help="Gioi han so luong tai moi phut"
    )
    parser.add_argument(
        "--schedule", help="Hen gio chay (dinh dang HH:MM, 24h)"
    )
    parser.add_argument(
        "--watchlist", help="File chua danh sach username (moi dong 1 username)"
    )
    parser.add_argument(
        "--self-check", action="store_true", help="Kiem tra moi truong va thoat"
    )
    parser.add_argument(
        "--verify", action="store_true", help="Kiem tra file checksum hien co va thoat"
    )
    parser.add_argument(
        "--yes",
        action="store_true",
        help="Tu dong chap nhan cac cau hoi (khong hoi lai)",
    )
    parser.add_argument(
        "--api", action="store_true", help="Du phong cho che do REST API trong tuong lai"
    )
    parser.add_argument(
        "--gui", action="store_true", help="Mo giao dien GUI thay vi chay trong terminal"
    )
...
def verify_checksums(root: Path, logger: Logger) -> None:
    ...
    for sha_path in root.rglob("*.sha256"):
        ...
        if expected != digest:
            ...
            issues += 1
    if issues == 0:
        logger.success("Tat ca file checksum deu hop le.")
    else:
        logger.warn(f"Da kiem tra checksum, co {issues} van de.")
...
def run_interactive(settings: Settings, logger: Logger, args: argparse.Namespace) -> None:
    ...
    ip_info = None if args.privacy else fetch_ip_metadata(
        session, settings.request_timeout
    )
...
def run_menu(settings: Settings, args: argparse.Namespace, logger: Logger) -> None:
    while True:
        ...
        print(
            f"{Theme.MUTED}2) Tai noi dung tu URL bat ky (YouTube, TikTok, ...){Theme.RESET}"
        )
        print(
            f"{Theme.MUTED}3) Upload video len TikTok (dung cookies/sessionid){Theme.RESET}"
        )
        print(
            f"{Theme.MUTED}4) Kiem tra file checksum trong thu muc tai ve{Theme.RESET}"
        )
        print(f"{Theme.MUTED}5) Thoat{Theme.RESET}")
        choice = input(
            f"{Theme.MUTED}Nhap lua chon (1-5): {Theme.PRIMARY}"
        ).strip()
